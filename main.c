 ...
 *
 * Sample string input:
 * 	.3.56..9......2.3..79..3.5..1.6..9..7.......3..6..7.1..2.7..64..9.1......5..29.78
 *
 * Authors: Jesse Deppisch, Joel Chavali
 */
...
int isSolved(Sudoku *s) {
	...
	/* Check blocks */
	for (int i = 1; i < 8; i += 3) { // Will check { 1, 4, 7 }
		for (int j = 1; j < 8; j += 3) {
			for (int n = 1; n < 10; n++) {
...
int checkBlock(Sudoku *s, int num, int i, int j) {
	...
	for (int x = 1; x < 8; x += 3) { // Will check { 1, 4, 7 }
		for (int y = 1; y < 8; y += 3) {
			if (abs(y - j) <= 1) {
				bCol = y;
			}
